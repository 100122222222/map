...
function initGUI(ambientLight){
  ...
  modelFolder.addColor(map.material,'color').name('color')
  // 布尔值设置单选框
  modelFolder.add(map.material, 'transparent').name('是否透明');
  // 下拉菜单、单选框
  modelFolder.add(map.position, 'x', [0, 1, 2, 4,8]);
  modelFolder.add(map.position, 'x', {
    min: -10,
    max: 10,
  });
